      ...
      <div className="relative w-full max-w-3xl px-4">
          ...
          {[
            ...
            <motion.div
              ...
              transition={{
                ...
                transform: { duration: 4, repeat: Infinity, ease: "easeInOut", repeatType: "reverse" }
              }}
            >
              <div className="group flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-xl rounded-full border border-white/20 hover:bg-white/20 transition-all duration-300">
                <span className="text-xl">{item.icon}</span>
                <span className="text-sm font-medium text-white opacity-80 group-hover:opacity-100">{item.text}</span>
              </div>
            </motion.div>
          ))}
        ...
        >
          {[
            ...
            <motion.div
              key={i}
              className="group px-6 py-4 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl border border-white/10 hover:border-white/20 transition-all duration-300"
              whileHover={{ y: -5, scale: 1.02 }}
              transition={{ duration: 0.2 }}
            >
              <div className="flex items-center gap-2">
                <span className="text-xl group-hover:scale-110 transition-transform duration-300">{item.icon}</span>
                <span className="text-sm font-medium text-white/80 group-hover:text-white transition-colors duration-300">{item.text}</span>
              </div>
            </motion.div>
          ))}
        </motion.div>
